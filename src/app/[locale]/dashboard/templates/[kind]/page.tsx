...
const KIND_LABELS: Record<string, { ar: string; en: string }> = {
  ...
  payment_receipt: { ar: 'إيصال قبض', en: 'Payment receipt' },
  expense_voucher: { ar: 'سند صرف', en: 'Expense voucher' },
...
export default function TemplateEditorPage({ params }: { params: Promise<{ kind: string }> }) {
  ...
  const label = KIND_LABELS[kind] ?? { ar: kind, en: kind };
  ...
  async function save() {
    ...
    setBusy(false);
    if (res.ok) toast.success(isAr ? 'تم حفظ القالب' : 'Template saved');
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={isAr ? `قالب ${label.ar}` : `${label.en} template`}
        description={isAr ? 'حرر تخطيط القالب مع معاينة فورية' : 'Edit layout with live preview'}
        actions={
          <Button onClick={save} disabled={busy}>
            <Save className="h-4 w-4" /> {busy ? (isAr ? 'جارٍ الحفظ…' : 'Saving…') : (isAr ? 'حفظ القالب' : 'Save layout')}
          </Button>
        }
      />
      ...
      <div className="grid gap-6 lg:grid-cols-[400px_1fr]">
        <Card>
          <CardHeader>
            <CardTitle>{isAr ? 'تحرير القالب' : 'Edit layout'}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              <div className="space-y-1.5">
                <Label>{isAr ? 'اسم القالب' : 'Layout name'}</Label>
                <Input value={name} onChange={(e) => setName(e.target.value)} />
              </div>

              <Tabs defaultValue="sections" className="w-full">
                <TabsList className="grid w-full grid-cols-3">
                  <TabsTrigger value="sections"><LayoutGrid className="me-1 h-3 w-3" /> {isAr ? 'الأقسام' : 'Sections'}</TabsTrigger>
                  <TabsTrigger value="styles"><Palette className="me-1 h-3 w-3" /> {isAr ? 'الأنماط' : 'Styles'}</TabsTrigger>
                  <TabsTrigger value="text"><Type className="me-1 h-3 w-3" /> {isAr ? 'النصوص' : 'Text'}</TabsTrigger>
                </TabsList>

                <TabsContent value="sections" className="space-y-4">
                  <Section title={isAr ? 'إظهار / إخفاء أقسام' : 'Show/Hide sections'}>
                    {SECTION_TOGGLES.map((s) => (
                      <Toggle key={s.k} label={isAr ? s.ar : s.en}
                        checked={!!layout.sections[s.k]}
                        onChange={(v) => set('sections', s.k, v)} />
                    ))}
                  </Section>
                  <Section title={isAr ? 'إخفاء أعمدة وحقول' : 'Hide columns & fields'}>
                    {HIDE_TOGGLES.map((s) => (
                      <Toggle key={s.k} label={isAr ? s.ar : s.en}
                        hint={isAr ? s.ar2 : s.en2}
                        checked={!!layout.sections[s.k]}
                        onChange={(v) => set('sections', s.k, v)} />
                    ))}
                  </Section>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Eye className="h-4 w-4" /> {isAr ? 'معاينة فورية' : 'Live preview'}</CardTitle>
          </CardHeader>
          <CardContent>
            <LivePreview layout={layout} kindLabel={isAr ? label.ar : label.en} />
          </CardContent>
        </Card>
      </div>
      ...
      {templates && templates.length > 1 && (
        <Card>
          <CardHeader><CardTitle>{isAr ? 'قوالب محفوظة' : 'Saved layouts'}</CardTitle></CardHeader>
          <CardContent>
            <ul className="divide-y">
              {templates.map((t) => (
                <li key={t.id} className="flex items-center justify-between py-2">
                  ...
                  {t.isDefault && <span className="text-xs text-muted-foreground">{isAr ? 'افتراضي' : 'Default'}</span>}
                </li>
              ))}
            </ul>
          </CardContent>
...
const SECTION_TOGGLES = [
  { k: 'companyHeader', ar: 'ترويسة الشركة', en: 'Company header' },
  { k: 'companyLogo', ar: 'شعار الشركة', en: 'Company logo' },
  { k: 'invoiceTitle', ar: 'عنوان وحالة الفاتورة', en: 'Invoice title & status' },
  { k: 'statusBadge', ar: 'شارة الحالة', en: 'Status badge' },
  { k: 'billTo', ar: 'قسم العميل', en: 'Bill-to section' },
  { k: 'invoiceDetails', ar: 'تفاصيل الفاتورة', en: 'Invoice details' },
  { k: 'itemsTable', ar: 'جدول البنود', en: 'Items table' },
  { k: 'totalsSection', ar: 'قسم الإجماليات', en: 'Totals section' },
  { k: 'shippingInfo', ar: 'بيانات الشحن', en: 'Shipping information' },
  { k: 'notesAndTerms', ar: 'ملاحظات وشروط', en: 'Notes & terms' },
  { k: 'footer', ar: 'التذييل', en: 'Footer' },
];

const HIDE_TOGGLES = [
  { k: 'hideDiscount', ar: 'إخفاء عمود الخصم', en: 'Hide discount column', ar2: 'إخفاء عمود الخصم في الفاتورة', en2: 'Hide discount column in invoice' },
  { k: 'hideTax', ar: 'إخفاء عمود الضريبة', en: 'Hide tax column', ar2: 'إخفاء عمود الضريبة في الفاتورة', en2: 'Hide tax column in invoice' },
  { k: 'hideCompanyEmail', ar: 'إخفاء إيميل الشركة', en: 'Hide company email', ar2: 'إخفاء البريد الإلكتروني للشركة', en2: 'Hide company email in invoice' },
  { k: 'hideCustomerEmail', ar: 'إخفاء إيميل العميل', en: 'Hide customer email', ar2: 'إخفاء البريد الإلكتروني للعميل', en2: 'Hide customer email in invoice' },
  { k: 'hidePaidAmount', ar: 'إخفاء المبلغ المدفوع', en: 'Hide paid amount', ar2: 'إخفاء المبلغ المسدّد', en2: 'Hide paid amount row' },
  { k: 'hideRemainingAmount', ar: 'إخفاء المتبقي', en: 'Hide remaining amount', ar2: 'إخفاء صف المتبقي', en2: 'Hide remaining amount row' },
  { k: 'hidePartyBalance', ar: 'إخفاء رصيد الجهة', en: 'Hide party balance', ar2: 'إخفاء رصيد العميل / المورد', en2: 'Hide customer/supplier balance' },
  { k: 'hideTotalAmount', ar: 'إخفاء الإجمالي', en: 'Hide total amount', ar2: 'إخفاء صف الإجمالي', en2: 'Hide total amount row' },
  { k: 'hideDueDate', ar: 'إخفاء تاريخ الاستحقاق', en: 'Hide due date', ar2: 'إخفاء تاريخ الاستحقاق في الفاتورة', en2: 'Hide due date in invoice' },
];

const TEXT_FIELDS = [
  { k: 'invoiceTitle', ar: 'عنوان الفاتورة', en: 'Invoice title' },
  { k: 'billToLabel', ar: 'تسمية المستلم', en: 'Bill To label' },
  { k: 'detailsLabel', ar: 'تسمية التفاصيل', en: 'Details label' },
  { k: 'itemsLabel', ar: 'تسمية البنود', en: 'Items label' },
  { k: 'subtotalLabel', ar: 'تسمية المجموع قبل الضريبة', en: 'Subtotal label' },
  { k: 'grandTotalLabel', ar: 'تسمية الإجمالي', en: 'Grand Total label' },
  { k: 'paidLabel', ar: 'تسمية المدفوع', en: 'Paid label' },
  { k: 'remainingLabel', ar: 'تسمية المتبقي', en: 'Remaining label' },
  { k: 'footerText', ar: 'نص التذييل', en: 'Footer text' },
];
